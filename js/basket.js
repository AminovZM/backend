...
window.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if (!response.ok) {
            ...
            const basketResponse = await fetch(`http://127.0.0.1:8000/baskets/?id_user=${user_id}`);
            ...
            const productPromises = basketData.map(async (item) => {
                ...
                const productResponse = await fetch(`http://127.0.0.1:8000/products/id?product_id=${productId}`);
            ...
            for (let i = 0; i < basketData.length; i++) {
                ...
                deleteButton.addEventListener('click', async () => {
                    try {
                        const response = await fetch(`http://127.0.0.1:8000/baskets/${user_id}/${productId}`, {
                            method: 'DELETE',
                            headers: {
...
async function createOrder(user_id, basketData) {
    try {
        ...
        if (response.ok) {
            ...
            fetch(`http://127.0.0.1:8000/baskets/?id_user=${user_id}`, {
                method: 'DELETE',
                headers: {
                    'accept': 'application/json'
                },
